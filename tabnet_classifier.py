...
def get_model(config):
    ...
    model=TabNet(input_dim=X.shape[1],output_dim=6,
                 n_d=config['n_d'],n_a=config['n_d'],n_steps=config['n_steps'],#gamma=config['gamma'],
                 cat_idxs=cat_idxs,cat_dims=cat_dims,cat_emb_dim=config['cat_emb_dim'],
                 n_independent=config['n_independent'],n_shared=config['n_shared'],
                 virtual_batch_size=config['virtual_batch_size'],#momentum=config['momentum'],
...
def train_fun(model,criterion,optimizer,train_loader,val_loader,lambda_sparsity,device='cpu',scheduler=None,
              l1_lambda=0.0,p_corrupt=0.0):
    # encoder_hook = OutputHook()
    # model.tabnet.encoder.register_forward_hook(encoder_hook)
    model.train()
    train_loss = 0
    total_train_loss=0
    ...
    class Trainer(tune.Trainable):
        ...
        def step(self):
            train_loss,total_train_loss,val_loss,total_val_loss,logloss=train_fun(self.model,self.criterion,
                                              ...
                                              val_loader=self.val_loader,lambda_sparsity=self.lambda_sparsity,device=device,
                                              scheduler=self.scheduler,l1_lambda=self.l1_lambda,p_corrupt=self.p_corrupt)
...
test_df_wide.to_csv(os.path.join(result_dir,"Tabnet baseline.csv"))
pseudo_labels_df.to_csv(os.path.join(result_dir,"Pseudo Labels - Tabnet.csv"))
